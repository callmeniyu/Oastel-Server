...
async function createBooking(req, res) {
    try {
        const { userId, packageType, packageId, date, time, adults, children, pickupLocation, contactInfo, subtotal, paymentInfo, isAdminBooking } = req.body;
        // Find or create time slot
        let timeSlot = await TimeSlot_1.default.findOne({
        ...
        try {
            await timeSlot_service_1.TimeSlotService.updateSlotBooking(packageType, new mongoose_1.Types.ObjectId(packageId), date, time, totalGuests, 'add');
        }
        catch (updateError) {
