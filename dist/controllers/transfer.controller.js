...
exports.getVehicles = exports.checkSlugAvailability = exports.updateTransferStatus = exports.deleteTransfer = exports.updateTransfer = exports.getLastTransfer = exports.getTransferBySlug = exports.getTransferById = exports.getTransfers = exports.createTransfer = void 0;
...
const createTransfer = async (req, res) => {
    try {
        ...
        if (!transferData.slug) {
            transferData.slug = (0, generateSlug_1.generateSlug)(transferData.title);
        }
        // Validate slug uniqueness
        const existingTransfer = await Transfer_1.default.findOne({ slug: transferData.slug });
        if (existingTransfer) {
        ...
        if (transferData.details && transferData.details.faq) {
            transferData.details.faq = transferData.details.faq.filter((faq) => faq.question.trim() && faq.answer.trim());
        }
        ...
        const transfer = new Transfer_1.default(transferData);
        ...
        try {
            await timeSlot_service_1.TimeSlotService.generateSlotsForPackage("transfer", savedTransfer._id, transferData.times || [], transferData.maximumPerson || 10);
...
const getTransfers = async (req, res) => {
    try {
        ...
        const transfers = await Transfer_1.default.find(query).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)).lean(); // Use lean() for better performance
        const total = await Transfer_1.default.countDocuments(query);
        res.json({
            success: true,
            data: transfers,
            pagination: {
...
const getTransferById = async (req, res) => {
    try {
        const { id } = req.params;
        const transfer = await Transfer_1.default.findById(id).lean(); // Use lean() for better performance
        if (!transfer) {
...
const getTransferBySlug = async (req, res) => {
    try {
        const { slug } = req.params;
        const transfer = await Transfer_1.default.findOne({ slug }).lean(); // Use lean() for better performance
        if (!transfer) {
            return res.status(404).json({
                success: false,
                message: "Transfer not found",
...
exports.getTransferBySlug = getTransferBySlug;
const getLastTransfer = async (req, res) => {
    try {
        const transfer = await Transfer_1.default.findOne({}).sort({ createdAt: -1 }).lean();
        if (!transfer) {
...
exports.getLastTransfer = getLastTransfer;
const updateTransfer = async (req, res) => {
    try {
        ...
        const existingTransfer = await Transfer_1.default.findById(id);
        if (!existingTransfer) {
            return res.status(404).json({
                success: false,
                message: "Transfer not found",
        ...
        const transfer = await Transfer_1.default.findByIdAndUpdate(id, transferData, { new: true, runValidators: true });
        try {
        ...
        try {
            ...
            if (JSON.stringify(times) !== JSON.stringify(existingTransfer.times) ||
                ...
                await timeSlot_service_1.TimeSlotService.updateSlotsForPackage("transfer", transfer._id, times, capacity);
...
const deleteTransfer = async (req, res) => {
    try {
        const { id } = req.params;
        const transfer = await Transfer_1.default.findByIdAndDelete(id);
        if (!transfer) {
            return res.status(404).json({
                success: false,
                message: "Transfer not found",
        ...
        try {
            await timeSlot_service_1.TimeSlotService.deleteSlotsForPackage("transfer", transfer._id);
...
const updateTransferStatus = async (req, res) => {
    try {
        ...
        const transfer = await Transfer_1.default.findByIdAndUpdate(id, { status }, { new: true, runValidators: true });
        if (!transfer) {
            return res.status(404).json({
                success: false,
                message: "Transfer not found",
...
const checkSlugAvailability = async (req, res) => {
    try {
        ...
        const existingTransfer = await Transfer_1.default.findOne(query);
        res.json({
            success: true,
...
const getVehicles = async (req, res) => {
    try {
        ...
        const vehicles = await Transfer_1.default.distinct("vehicle", {
            type: "Private",
