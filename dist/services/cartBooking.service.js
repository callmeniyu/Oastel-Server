...
exports.cartBookingService = exports.CartBookingService = void 0;
...
class CartBookingService {
    ...
    async bookCartItems(request) {
        ...
        try {
            ...
            const user = await User_1.default.findOne({ email: request.userEmail }).session(session);
            if (!user) {
            ...
            const cart = await Cart_1.default.findOne({ userId: user._id }).session(session);
            ...
            for (const item of cart.items) {
                try {
                    ...
                    if (itemDate < currentDate) {
                        result.warnings.push(`${item.packageTitle}: Selected date (${itemDate.toLocaleDateString()}) has already passed. Skipping this item.`);
                        continue;
                    }
                    // Package availability validation
                    let packageDoc;
                    if (item.packageType === 'tour') {
                        packageDoc = await Tour_1.default.findById(item.packageId).session(session);
                    }
                    else {
                        packageDoc = await Transfer_1.default.findById(item.packageId).session(session);
                    }
                    if (!packageDoc) {
                    ...
                    if (existingBooking) {
                        result.warnings.push(`${item.packageTitle}: You already have a booking for this date and time. Skipping this item.`);
                        continue;
                    }
                    ...
                    try {
                        await timeSlot_service_1.TimeSlotService.updateSlotBooking(item.packageType, item.packageId, new Date(item.selectedDate).toISOString().split('T')[0], item.selectedTime, totalGuests, "add");
                    ...
                    try {
                        if (item.packageType === 'tour') {
                            await Tour_1.default.findByIdAndUpdate(item.packageId, { $inc: { bookedCount: totalGuests } }, { session });
                            console.log(`✅ Updated Tour bookedCount by ${totalGuests} for package ${item.packageId}`);
                        }
                        else if (item.packageType === 'transfer') {
                            await Transfer_1.default.findByIdAndUpdate(item.packageId, { $inc: { bookedCount: totalGuests } }, { session });
                    ...
                    if (itemError?.name === 'ValidationError') {
                        ...
                        const validationMessages = Object.values(itemError.errors || {}).map((err) => err.message).join(', ');
            ...
            if (result.bookings.length > 0) {
                ...
                await Cart_1.default.findOneAndUpdate({ userId: user._id }, { $set: { items: [] } }, { session });
                result.success = true;
                ...
                try {
                    ...
                    const cartEmailData = {
                        ...
                        bookings: cartItemsForEmail.map((cartItem, index) => {
                        ...
                        totalAmount: cartItemsForEmail.reduce((sum, item) => {
    ...
    async getCartBookingSummary(userEmail) {
        try {
...
exports.CartBookingService = CartBookingService;
exports.cartBookingService = new CartBookingService();
