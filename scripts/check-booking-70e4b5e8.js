...
async function checkBooking() {
  try {
    ...
    if (booking) {
      ...
      console.log('Adults:', booking.numberOfAdults);
      console.log('Children:', booking.numberOfChildren);
      
      console.log('\n--- Full Booking Object ---');
      console.log(JSON.stringify(booking, null, 2));
    } else {
      console.log('❌ Booking not found with ID containing 70E4B5E8');
      console.log('\n--- Searching all bookings for Clara Beneitez ---');
      const claraBookings = bookings.filter(b => 
        b.contactInfo?.name?.toLowerCase().includes('clara') ||
        b.contactInfo?.email?.toLowerCase().includes('clarabeneitez')
      );
      console.log(`Found ${claraBookings.length} bookings for Clara Beneitez:`);
      claraBookings.forEach(b => {
        console.log(`\nID: ${b._id.toString()}`);
        console.log(`Date: ${b.date} (${new Date(b.date).toLocaleDateString('en-CA', { timeZone: 'Asia/Kuala_Lumpur' })})`);
